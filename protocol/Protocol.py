...
class Protocol:
    ...
    def _new(self, client):
        login_string = ' '.join((self._root.server, str(self._root.server_version), self._root.latestspringversion,
                                 str(self._root.natport), '0'))
    ...
    def _remove(self, client, reason='Quit'):
        ...
        logging.info(
            '[%s] <%s> disconnected from %s: %s' % (client.session_id, client.username, client.ip_address, reason))
    ...
    def _informErrors(self, client):
        if client.lobby_id in ("SpringLobby 0.188 (win x32)", "SpringLobby 0.200 (win x32)"):
            client.Send(
                "SAYPRIVATE ChanServ The autoupdater of SpringLobby 0.188 is broken, please manually update: http://springrts.com/phpbb/viewtopic.php?f=64&t=31224")
    ...
    def in_LOGIN(self, client, username, password='', cpu='0', local_ip='', sentence_args=''):
        ...
        good, user_or_error = self.userdb.legacy_login_user(username, password, client.ip_address, lobby_id, user_id,
                                                            cpu, local_ip, client.country_code)
        ...
        if (client.access == 'agreement'):
            ...
            logging.info(
                '[%s] Sent user <%s> the terms of service on session.' % (client.session_id, user_or_error.username))
    ...
    def _SendLoginInfo(self, client):
        ...
        if client.status != 0:
            self._root.broadcast(
                'CLIENTSTATUS %s %d' % (client.username, client.status))  # broadcast current client status
    ...
    def in_JOIN(self, client, chan, key=None):
        ...
        if client.compat['o']:
            for msg in msgs:
                client.Send("SAID " + self._dictToTags(
                    {"chanName": chan, "time": msg[0].isoformat(), "userName": msg[1], "msg": msg[2]}))
        else:
            for msg in msgs:
    ...
    def in_OPENBATTLE(self, client, type, natType, password, port, maxplayers, hashcode, rank, maphash, sentence_args):
        ...
        try:
            ...
            self.out_OPENBATTLEFAILED(client,
                                      'Invalid argument type, send this to your lobby dev: id=%s type=%s natType=%s passworded=%s port=%s maphash=%s gamehash=%s - %s' %
                                      (battle_id, type, natType, passworded, port, maphash, hashcode,
                                       str(e).replace("\n", "")))
            return False
        ...
        if maxplayers > 10 and not client.bot:
            maxplayers = 10
            self.out_SERVERMSG(client,
                               "Without botflag its not allowed to host battles with > 10 players. Your battle was restricted to 10 players")
    ...
    def _joinBattle(self, client, battle):
        ...
        for iter in battle_bots:
            ...
            client.Send(
                'ADDBOT %s %s' % (battle.id, iter) + ' %(owner)s %(battlestatus)s %(teamcolor)s %(AIDLL)s' % (bot))
        for allyno in startrects:
            rect = startrects[allyno]
            client.Send('ADDSTARTRECT %s' % (allyno) + ' %(left)s %(top)s %(right)s %(bottom)s' % (rect))
        client.battlestatus = {'ready': '0', 'id': '0000', 'ally': '0000', 'mode': '0', 'sync': '00', 'side': '00',
                               'handicap': '0000000'}
        client.teamcolor = '0'
    ...
    def in_JOINBATTLEACCEPT(self, client, username):
        ...
        battle = self._root.battles[battle_id]
        if not client.session_id == battle.host:
            self.out_FAILED(client, 'JOINBATTLEACCEPT',
                            "client isn't the specified host %s vs %s" % (client.session_id, battle.host), True)
            return
        if not user.session_id in battle.pending_users:
            self.out_FAILED(client, 'JOINBATTLEACCEPT',
                            "client isn't in pending users %s %s" % (client.username, username), True)
            return
    ...
    def in_SETSCRIPTTAGS(self, client, scripttags):
        ...
        battle = self._canForceBattle(client)
        if not battle:
            self.out_FAILED(client, "SETSCRIPTTAGS", "You are not allowed to change settings as client in a game!",
                            True)
            return
    ...
    def in_LEAVEBATTLE(self, client):
        ...
        if oldspecs != specs:
            self._root.broadcast('UPDATEBATTLEINFO %s %i %i %s %s' % battle.id, battle.spectators, battle.locked,
                                 battle.maphash, battle.map)
    ...
    def in_MYBATTLESTATUS(self, client, _battlestatus, _myteamcolor):
        ...
        if battlestatus < 0:
            self.out_FAILED(client, 'MYBATTLESTATUS',
                            'invalid status is below 0: %s. Please update your lobby!' % (_battlestatus), True)
        ...
        u, u, u, u, side1, side2, side3, side4, sync1, sync2, u, u, u, u, handicap1, handicap2, handicap3, handicap4, handicap5, handicap6, handicap7, mode, ally1, ally2, ally3, ally4, id1, id2, id3, id4, ready, u = self._dec2bin(
            battlestatus, 32)[-32:]
        ...
        client.battlestatus.update(
            {'ready': ready, 'id': id1 + id2 + id3 + id4, 'ally': ally1 + ally2 + ally3 + ally4, 'mode': mode,
             'sync': sync1 + sync2, 'side': side1 + side2 + side3 + side4})
    ...
    def in_ADDSTARTRECT(self, client, allyno, left, top, right, bottom):
        ...
        self._root.broadcast_battle('ADDSTARTRECT %s' % (allyno) + ' %(left)s %(top)s %(right)s %(bottom)s' % (rect),
                                    client.current_battle, [client.username])
    ...
    def in_ADDBOT(self, client, name, battlestatus, teamcolor, AIDLL):
        ...
        battle.bots[name] = {'owner': client.username, 'battlestatus': battlestatus, 'teamcolor': teamcolor,
                             'AIDLL': AIDLL}
        self._root.broadcast_battle(
            'ADDBOT %s %s %s %s %s %s' % (battle_id, name, client.username, battlestatus, teamcolor, AIDLL), battle_id)
        ...
        if battle_id in self._root.battles:
            ...
            if name in battle.bots:
                if client.username == battle.bots[name]['owner'] or client.session_id == battle.host:
                    ...
                    self._root.broadcast_battle('UPDATEBOT %s %s %s %s' % (battle_id, name, battlestatus, teamcolor),
                                                battle_id)
    ...
    def in_GETIP(self, client, username):
        ...
        if username in self._root.usernames:
            self.out_SERVERMSG(client,
                               '<%s> is currently bound to %s' % (username, self._root.usernames[username].ip_address))
            return
    ...
    def in_RENAMEACCOUNT(self, client, newname):
        ...
        if good:
            self.out_SERVERMSG(client,
                               'Your account has been renamed to <%s>. Reconnect with the new username (you will now be automatically disconnected).' % newname)
    ...
    def in_KICKUSER(self, client, user, reason=''):
        ...
        if user in self._root.usernames:
            ...
            for chan in kickeduser.channels:
                self._root.broadcast(
                    'CHANNELMESSAGE %s <%s> kicked <%s> from the server%s' % (chan, client.username, user, reason),
                    chan)
    ...
    def in_CLEANUP(self, client):
        self._root.admin_broadcast(
            'Cleanup initiated by %s (deleting old users, zombie channels / users)...' % (client.username))
        nchan = 0
        nbattle = 0
        nuser = 0
        # cleanup battles
